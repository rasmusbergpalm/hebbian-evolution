...
class Car:
    def __init__(self, world, init_angle, init_x, init_y, side_force=0.0, friction=1.0):
        self.world = world
        self.side_force = side_force
        self.friction_multiplier = friction
    ...
    def step(self, dt):
        for w in self.wheels:
            ...
            grass = True
            friction_limit = self.friction_multiplier * FRICTION_LIMIT * 0.6  # Grass friction if no tile
            for tile in w.tiles:
                friction_limit = max(friction_limit, self.friction_multiplier * FRICTION_LIMIT * tile.road_friction)
                grass = False
