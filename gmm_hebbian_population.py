...
class GMMHebbianPopulation(Population):
    def __init__(self, num_learning_rules: int):
    ...
    def sample(self, n) -> Iterable[HebbianAgent]:
        return [
            HebbianAgent.from_params({
                key: d.Normal(loc=self.learning_rule_cluster_means[d.Categorical(logits=mixing_logits_tensor).sample()], scale=self.sigma).sample()
    ...
    def log_prob(self, individual: HebbianAgent) -> float:
        ...
        for key, h in individual.get_params().items():
            ...
            p_h_given_k = d.Normal(loc=self.learning_rule_cluster_means, scale=self.sigma).expand((n_in, n_out, M, 5)).sg((n_in, n_out, "M", 5))
