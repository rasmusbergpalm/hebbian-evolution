...
from hebbian_agent import StaticCarRacingAgent


class NormalHebbianPopulation(Population):
    def __init__(self, scale):
        self.scale = scale
        self.learning_rule_means = {k: t.randn(v.shape, requires_grad=True) for k, v in StaticLunarLanderAgent().get_params().items()}
    ...
    def sample(self, n) -> Iterable[StaticLunarLanderAgent]:
        ...
        for _ in range(n // 2):
            ...
            agents.append(StaticLunarLanderAgent.from_params({k: self.learning_rule_means[k] + n for k, n in noise.items()}))
            agents.append(StaticLunarLanderAgent.from_params({k: self.learning_rule_means[k] - n for k, n in noise.items()}))
